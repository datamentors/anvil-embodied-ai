...
def make_two_arm_node() -> LeRobotInferenceNode:
    ...
    node._allow_live_joint_limit_saturation = False
...
def make_finger_saturation_node() -> LeRobotInferenceNode:
    node = object.__new__(LeRobotInferenceNode)
    node.arms_config = {
        "left": {"action_start": 0, "action_end": 2, "ros_prefix": "follower_l"}
    }
    node.joint_names_config = {
        "model_joint_order": ["finger_joint1", "joint1"],
        "controller_joint_order": ["joint1", "finger_joint1"],
    }
    positions = {
        "follower_l_joint1": 0.0,
        "follower_l_finger_joint1": 0.049,
    }
    node.strategy = SimpleNamespace(get_current_joint_positions=lambda: positions)
    ...
    node._joint_limit_tolerance = 1e-6
    node._joint_limit_saturation_margins = {
        "follower_l_finger_joint1": (0.003, 0.003)
    }
    node._saturate_all_raw_targets_to_joint_limits = False
    node._joint_limit_saturation_counts = {}
    node._joint_limit_saturation_last_log = {}
    node.action_limiter = ActionLimiter(
        max_delta=0.04,
        model_joint_order=["finger_joint1", "joint1"],
        controller_joint_order=["joint1", "finger_joint1"],
    )
    node.arm_publishers = {"left": FakePublisher()}
    node._monitor_enable = False
    node._smooth_tracker = None
    node._debug = False
    node.metrics = FakeMetrics()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("unknown_finger_joint1", [0.001, 0.001], "unknown joints"),
        ("follower_l_finger_joint1", [0.003001, 0.0], r"within \[0, 0.003\]"),
        ("follower_l_finger_joint1", [0.0, 0.0], "one must be non-zero"),
    ],
)
def test_invalid_joint_saturation_margin_rejected(
    joint_name, margins, expected
) -> None:
    node = make_finger_saturation_node()

    with pytest.raises(ValueError, match=expected):
        node._parse_joint_limit_saturation_margins({joint_name: margins})


def test_revolute_saturation_margin_requires_debug_or_explicit_live_opt_in() -> None:
    node = make_finger_saturation_node()
    node.arms_config["left"]["command_topic"] = (
        "/follower_l_forward_position_controller/commands"
    )

    with pytest.raises(ValueError, match="allow_live_joint_limit_saturation=true"):
        node._parse_joint_limit_saturation_margins(
            {"follower_l_joint1": [0.01, 0.0]}
        )

    node._allow_live_joint_limit_saturation = True
    assert node._parse_joint_limit_saturation_margins(
        {"follower_l_joint1": [0.01, 0.0]}
    ) == {"follower_l_joint1": (0.01, 0.0)}
...
def test_revolute_saturation_margin_is_capped_at_one_hundred_twenty_milliradians() -> None:
    node = make_finger_saturation_node()
    node.arms_config["left"]["command_topic"] = (
        "/debug/follower_l_forward_position_controller/commands"
    )
    ...
    assert node._parse_joint_limit_saturation_margins(
        {"follower_l_joint1": [0.12, 0.0]}
    ) == {"follower_l_joint1": (0.12, 0.0)}
    with pytest.raises(ValueError, match=r"within \[0, 0.12\]"):
        node._parse_joint_limit_saturation_margins(
            {"follower_l_joint1": [0.120001, 0.0]}
        )


def test_finger_shadow_diagnostic_margin_is_capped_at_ten_millimetres() -> None:
    node = make_finger_saturation_node()
    node.arms_config["left"]["command_topic"] = (
        "/debug/follower_l_forward_position_controller/commands"
    )
    ...
    with pytest.raises(ValueError, match=r"within \[0, 0.01\]"):
        node._parse_joint_limit_saturation_margins(
            {"follower_l_finger_joint1": [0.010001, 0.0]}
        )


def test_finger_target_is_saturated_before_delta_limiter() -> None:
    node = make_finger_saturation_node()

    # Model order is finger, joint1; controller order is joint1, finger.
    node._publish_action(np.array([0.050483163, 0.2]))

    # joint1 is delta-limited to 0.04 and the finger is hard-clipped to 0.05.
    assert node.arm_publishers["left"].messages == [[0.04, 0.05]]
    assert node._joint_limit_saturation_counts == {
        "follower_l_finger_joint1": 1
    }
    assert node._joint_limit_saturation_max_overshoot == pytest.approx(
        {"follower_l_finger_joint1": 0.000483163}
    )
    assert node._has_published


def test_finger_target_beyond_explicit_margin_fails_closed() -> None:
    node = make_finger_saturation_node()

    with pytest.raises(ValueError, match="raw absolute.*finger_joint1"):
        node._publish_action(np.array([0.053001, 0.2]))

    assert node.arm_publishers["left"].messages == []
    assert not node._has_published


def test_diagnostic_raw_saturation_never_expands_final_command_limits() -> None:
    node = make_finger_saturation_node()
    node._joint_limit_saturation_margins = {}
    node._saturate_all_raw_targets_to_joint_limits = True

    # Model order is finger, joint1. Arbitrary raw targets are first clipped to
    # the configured hard limits, then rate-limited against measured state.
    node._publish_action(np.array([0.50, 5.0]))
    ...
    # The diagnostic exception is deliberately raw-only. A final command that
    # lies outside the same hard map still fails closed.
    with pytest.raises(ValueError, match="outside absolute limit"):
        node._sanitize_absolute_joint_targets(
            ["follower_l_joint1"],
            np.array([1.01]),
            stage="final command",
        )
