...
class BufferedStreamExtractor:
    ...
    def __init__(
        ...
        Args:
            ...
            progress_every: Emit callback/console progress every N yielded frames
        """
        self.config = config
        self.fps = fps
        ...
        # Cache for action topic reorder permutations: {topic: np.ndarray}
        self._action_reorder_cache: Dict[str, np.ndarray] = {}
        self._joint_layout_cache: Dict[Tuple[str, ...], Dict[Tuple[str, str], Dict[str, Any]]] = {}
    ...
    def _find_nearest_in_buffer(
        ...
        # Buffer timestamps are monotonic. Once the search passes the target and
        # the distance starts increasing, the best match is already behind us.
        min_diff = float("inf")
        nearest_idx = 0
    ...
    def _emit_progress(self, frames_yielded: int, force: bool = False) -> None:
        """Throttle progress callback churn inside the hot extraction loop."""
        if self.progress_callback:
            if (
                force
                or self._last_reported_frames < 0
                or frames_yielded - self._last_reported_frames >= self.progress_every
            ):
    ...
    def _compile_joint_layout(
        ...
        """Compile one stable JointState name layout into sorted gather indices."""
        ...
        for idx, joint_name in enumerate(joint_names):
            ...
            if result is None:
                continue
            role, robot, joint_id = result
            grouped.setdefault((role, robot), []).append((joint_id, idx))

        layout: Dict[Tuple[str, str], Dict[str, Any]] = {}
        for key, pairs in grouped.items():
            pairs.sort(key=lambda item: item[0])
            layout[key] = {
                "joint_names": [joint_id for joint_id, _ in pairs],
                "indices": np.array([idx for _, idx in pairs], dtype=np.intp),
            }
        return layout

    def _gather_joint_field(self, values: Any, indices: np.ndarray) -> np.ndarray:
        """Gather a joint field into canonical order with a fast numpy path."""
        if values is None or len(values) == 0 or indices.size == 0:
            return np.array([], dtype=np.float32)

        field = np.asarray(values, dtype=np.float32)
        max_index = int(indices[-1])
        if field.shape[0] > max_index:
            return field[indices]

        valid = indices[indices < field.shape[0]]
        if valid.size == 0:
            return np.array([], dtype=np.float32)
        return field[valid]
    ...
    def _buffer_joint_state(
        ...
        names_key = tuple(ros_msg.name)
        layout = self._joint_layout_cache.get(names_key)
        if layout is None:
            layout = self._compile_joint_layout(names_key)
            self._joint_layout_cache[names_key] = layout
        ...
        for key, data in layout.items():
            ...
            pos = self._gather_joint_field(ros_msg.position, indices)
            vel = self._gather_joint_field(ros_msg.velocity, indices)
            eff = self._gather_joint_field(ros_msg.effort, indices)
