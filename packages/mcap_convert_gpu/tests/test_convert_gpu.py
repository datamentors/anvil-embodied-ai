...
from mcap_convert_gpu.cli.convert import build_parser
from mcap_convert_gpu.core.writer import LeRobotWriter
...
def test_gpu_parser_keeps_auto_available_when_requested():
    parser = build_parser(profile="gpu")
    args = parser.parse_args(["-i", "/tmp/input", "-o", "/tmp/out", "--vcodec", "auto"])
...
def test_writer_create_forwards_streaming_encoder_args(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("mcap_convert_gpu.core.writer.LeRobotDataset.create", fake_create)
...
def test_writer_resume_forwards_streaming_encoder_args(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("mcap_convert_gpu.core.writer.LeRobotDataset.resume", fake_resume)
    monkeypatch.setattr("mcap_convert_gpu.core.writer._patch_resume_video_continuation", lambda dataset: None)
